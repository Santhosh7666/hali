...
import { BarChart3, Loader2, User, Mail, Lock, ArrowRight, ChevronLeft } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-indigo-500/30 font-sans flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-violet-600/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-lg relative z-10 text-left">
        ...
        >
          ...
          <h2 className="text-4xl font-black tracking-tight mb-3">{t('auth.registerTitle')}</h2>
          <p className="text-slate-400 font-medium">{t('auth.registerDesc')}</p>
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-1">{t('auth.name')}</label>
              <div className="relative group">
                <User className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
                <input
                  type="text"
                  value={name}
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-1">{t('auth.password')}</label>
              <div className="relative group">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-4 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-xl shadow-indigo-600/20 disabled:opacity-50 group hover:scale-[1.02] active:scale-[0.98]"
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                <>
                  {t('auth.signUp')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </form>
